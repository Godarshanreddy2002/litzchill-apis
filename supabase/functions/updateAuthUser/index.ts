...
Deno.serve(async (req) => {
  try {
    ...
    if(data)
    {
      ...
      {
        return new Response(JSON.stringify("User Account is deactivated/Suspended"),
        {
    ...
    if (userUError) {
      return new Response(`Error updating profile: ${userUError.message}`, { status: 400 });
    }

    
    return new Response(JSON.stringify("User data is successfully updated"), 
    {
      status: 200,
